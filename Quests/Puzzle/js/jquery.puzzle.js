$(document).ready(// When the document is ready, hook up the puzzle
	...
	return this.filter( ":has( img )" ).each(
		...
		function( intI ){
			...
			function InitPuzzle(){
				...
				for (intI = 0 ; intI < Math.min(50*intRows*intColumns,1000); intI++){
			...
			function partCalc() {
                return parseInt(jImg.width() / intSize) * parseInt(jImg.height() / intSize);
            }
			function gcd(a,b) {
                if (a === 0) return b;
                return gcd(b % a, a);
